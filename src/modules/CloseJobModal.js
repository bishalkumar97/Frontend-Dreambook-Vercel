...
export default function CloseJobModal({handler,data,yesHandler,loading}) {
    return (
        <div className='modal-container'>
            ...
            <h2 className='font-sora text-2xl text-center w-full font-semibold'>
                Are you sure you want to {data?.is_closed ? data?.is_closed == 0 ?'Close Job' :'Open Job Again' : "Close job"}?
            </h2>
            ...
            <div className='w-full grid grid-cols-2 mt-5 gap-3'>
                ...
                <Button variant="danger" className='bg-purple-400' loading={loading} onClick={yesHandler}>Yes</Button>
            </div>
            </div>
        </div>
    )
